-- Corner pin effect filter LUA script OBS dleiferives
-- Based off Corner Pin effect filter v1.4 by khaver
-- Was causing tearing on my machine so I cleaned up the shader code
-- otherwise its the same as https://obsproject.com/forum/resources/corner-pin.1474/ v1.4
...
{
    float2 a = float2(pos_tlx, pos_tly);
    float2 b = float2(1.0 - pos_trx, pos_try);
    float2 c = float2(1.0 - pos_brx, 1.0 - pos_bry);
    float2 d = float2(pos_blx, 1.0 - pos_bly);
    ...
    if (abs(k2) < eps){
        if (abs(k1) < eps) return float2(-1.0, -1.0);
        v1 = -k0 / k1;
        float denom_x = e.x + g.x * v1;
        float denom_y = e.y + g.y * v1;
        u1 = (abs(denom_x) > abs(denom_y))
            ? (h.x - f.x * v1) / denom_x
            : (h.y - f.y * v1) / denom_y;
        if (v1 > -eps && v1 < 1.0+eps && u1 > -eps && u1 < 1.0+eps)
            return float2(u1, v1);
    ...
    else
    {
        ...
        float denom1x = e.x + g.x * v1;
        float denom1y = e.y + g.y * v1;
        u1 = (abs(denom1x) > abs(denom1y))
            ? (h.x - f.x * v1) / denom1x
            : (h.y - f.y * v1) / denom1y;

        float denom2x = e.x + g.x * v2;
        float denom2y = e.y + g.y * v2;
        u2 = (abs(denom2x) > abs(denom2y))
            ? (h.x - f.x * v2) / denom2x
            : (h.y - f.y * v2) / denom2y;
    }
    bool  b1 = v1>-eps && v1<1.0+eps && u1>-eps && u1<1.0+eps;
    bool  b2 = v2>-eps && v2<1.0+eps && u2>-eps && u2<1.0+eps;

    float2 res = float2(-1.0, -1.0);

    if ( b1 && b2 ) {
        float d1 = abs(u1-0.5) + abs(v1-0.5);
        float d2 = abs(u2-0.5) + abs(v2-0.5);
        return (d1 < d2) ? float2(u1, v1) : float2(u2, v2);
    }
